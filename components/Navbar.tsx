      ...
      <nav className="bg-white text-gray-900 shadow-md border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center hover:opacity-80 transition-opacity">
            <Image src="/logo.jpg" alt="盛源 Persona" width={160} height={40} className="h-10 w-auto" />
          </Link>
            ...
            {links.map((link) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-yellow-500 ${
                  pathname === link.href ? "text-yellow-500" : "text-gray-700"
                }`}
              >
                {link.label}
              </Link>
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="md:hidden bg-gray-50 border-t border-gray-200 px-4 py-3 flex flex-col gap-3">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
