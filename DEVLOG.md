...
**變更：**
- `app/products/page.tsx`：LED 產品下方新增觸控廣告機專區，55"/65" 雙機型規格對照表 + 用途標籤 + LINE 洽詢 CTA
- `messages/zh-TW.json` + `messages/en.json`：新增 kiosk_* 系列 i18n 鍵值（共 19 組）

**規格來源：** 型錄 55K10SK-PRO / 65K10SK-PRO
- 55"：1984×745×389mm / 4K / 350cd/m² / IR觸控20點 / Android 13 / 33.9kg / ≤120W / 庫存28台
- 65"：1999×821×730mm / 4K / 350cd/m² / IR觸控20點 / Android 13 / 64.7kg / 庫存2台

**原因：**
- 擴充產品線至觸控廣告機；租金TBD故採LINE洽詢流程，不接入試算器
...
**變更：**
- `app/calculator/CalculatorClient.tsx`：電梯勾選下方依機型顯示外包裝尺寸（兩折機：84×102.5×130.5cm；三折機：確認中）
- `messages/zh-TW.json` + `messages/en.json`：新增 `elevator_dim_single` / `elevator_dim_triple` 鍵值

**原因：**
- 客戶需要知道外包裝實際尺寸才能判斷電梯能否進入，否則勾選「有電梯」結論不準確
...
- `messages/zh-TW.json` + `messages/en.json`：
  - `panel1/3_label`：標注 172 × 1032
  - `panel2_label`：標注 344 × 1032
  - `triple_footer`：加入各面板尺寸標注
  - `triple_tips_a`：背面A改為 344×1032（A右半172＋A左半172）
  - `triple_tips_b`：正面B改為 344×1032（原誤標 688×1032）
- `app/guide/GuideClient.tsx`：
  - `W` 拆分為 `W1=35`（面板1/3）和 `W2=70`（面板2）
  - 平面展開圖中面板2顯示為面板1/3的2倍寬，符合實際比例

**原因：**
- Jeff 與美工同事確認：三折機全展開 688×1032，其中面板2為344×1032（中央正面），面板1和3各為172×1032（兩側折疊為背面）。之前誤以為三個面板等寬

---

## v1.4.3 — 2026-05-08：修正兩折機素材規格說明 + 三折機背面組成標注

**變更：**
- `messages/zh-TW.json` + `messages/en.json`：
  - 兩折機：`single_desc`、`back_label`、`front_label`、`single_tips_a/b/note` 全面更正
  - 三折機：`triple_tips_a` 加注 A右半＋A左半各 344×1032 組成

**原因：**
- Jeff 確認兩折機規格有誤：機器對折後每面實際解析度為 344×516，並非 344×1032。客戶應設計一個 344×1032 畫布，上下各放一段 344×516 影片且頭對頭排列，而非兩個各自獨立的全高影片
- 三折機背面（A右半＋A左半）需標注各為 344×1032 獨立面板，讓客戶了解拼接結構
...
**變更：**
- 新增 `app/guide/page.tsx` + `app/guide/GuideClient.tsx`：素材製作指南互動頁面
- 兩折機分頁：選取 A/B 兩段素材後，即時呈現平面展開示意圖（背面素材以 rotate 180° 呈現）與折疊後正背面效果預覽
- 三折雙面機分頁：選取 A/B 兩段素材後，呈現三面板平面圖（面板 1 水平鏡像）與折疊後效果預覽
- `components/Navbar.tsx`：導覽列新增「素材指南」連結（在拼接模擬後）
- `messages/zh-TW.json` + `messages/en.json`：加入 nav.guide 翻譯

**原因：**
- 客戶常問「影片要怎麼製作」，尤其背面旋轉 180° 和三折鏡像規則需要視覺化說明

---

## v1.3.4 — 2026-05-07：模擬器功耗顯示 + 預約截止邏輯

**變更：**
- `app/simulator/SimulatorClient.tsx`：MACHINE_CONFIG 加入 `maxPowerW`（兩折機 600W / 三折雙面機 1200W），規格欄依台數動態顯示最大消耗電力；超過 1500W 顯示紅色跳電警示
- `app/booking/BookingClient.tsx`：中午 12 點後封鎖明日預約（最早可選後天）；中午前預約明日時顯示「請於今日 17:00 前完成匯款」提醒
...
**變更：**
- `components/PromoBanner.tsx`：`PROMO_ENABLED` 從 `true` 改為 `false`

**原因：**
- 促銷活動內容尚在構思中，暫時隱藏整個輪播欄位，待確定促銷方案後再啟用

---

## v1.3.1 — 2026-05-06：嵌入頁標題修正

**變更：**
- `led-rental-embed.html`：`<title>` 從「LED廣告機租賃 | CS Knight」改為「LED廣告機租賃 | 盛源Persona」

**原因：**
- 嵌入頁顯示在官網底下，連結預覽標題需顯示正確品牌名稱
